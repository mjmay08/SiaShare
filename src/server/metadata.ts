...
export class Metadata {
  ...
  public async addFileToRoom(roomId: string, tusId: string, fileId: string, status: boolean) {
    ...
    await this.db.exec(
      `INSERT INTO files (tusId, roomId, fileId, status) VALUES('${tusId}', '${roomId}', '${fileId}', ${statusInt});`
    );
  }
  ...
  public async updateFileStatus(roomId: string, tusId: string, fileId: string, status: boolean) {
    const statusInt: number = status ? 1 : 0;
    await this.db.exec(
      `UPDATE files SET status = ${statusInt} WHERE tusId = '${tusId}' AND roomId = '${roomId}' AND fileId = '${fileId}';`
    );
  }
